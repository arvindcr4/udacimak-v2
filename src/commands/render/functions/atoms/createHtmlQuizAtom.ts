 ...
 */
export default async function createHtmlQuizAtom(
  ...
  try {
    ...
    let promiseQuizQuestion: Promise<string>;
    let promiseQuizUserAnswer: Promise<string | undefined> = Promise.resolve(undefined);
    ...
    if (semanticType === 'ProgrammingQuestion' || semanticType === 'IFrameQuestion') {
      ...
      promiseQuizQuestion = createHtmlQuizImageFormQuestion(atom, targetDir);
    } else {
